...
async function main() {
  ...
  const active_idx: number[] = [];
  events.forEach((e) => {
    const arg = e.args || { _index: 0 };
    active_idx.push(arg._index.toNumber());
  })

  while (active_idx.length > 0) {
    await Promise.all(active_idx.map(async (instance, idx) => {
      ...
      switch (parsedState) {
        case "ConsensusResult":
          active_idx.splice(idx, 1);
          break;
        case "ChallengerWon":
          active_idx.splice(idx, 1);
          failedTests.push(`Descartes test at index ${instance} has failed. Finished at ${parsedState} state.`);
          break;
        case "ClaimerWon":
          active_idx.splice(idx, 1);
          failedTests.push(`Descartes test at index ${instance} has failed. Finished at ${parsedState} state.`);
  ...
  if (failedTests.length > 0) {
    throw (`Some tests failed. Here is the compiled messages: ${JSON.stringify(failedTests, null, 2)}`);
  }
}
...
const timeout = new Promise((_, rejects) => {
  ...
  setTimeout(() => {
    rejects(`The test has timedout after waiting for ${time / (1000 * 60)} min. Check the logs.`);
  }, time);
})
