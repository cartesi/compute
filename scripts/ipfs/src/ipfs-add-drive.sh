...
output=$(docker run \
  ...
  -v `pwd`:/opt/cartesi/srv/compute \
  --rm  $CARTESI_IPFS_DOCKER \
  -address $IPFS_SERVICE_ADDRESS -mode add -argument /opt/cartesi/srv/compute/flashdrive/$INPUT_DRIVE_FILENAME 2>&1)
...
IPFS_PATH=$(echo -n $IPFS_PATH | sed 's/\"}//g')
